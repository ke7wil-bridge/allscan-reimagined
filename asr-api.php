...
function asr_diag_command(string $command, int $maxChars = 4000): string {
    ...
    return strlen($output) > $maxChars ? substr($output, -$maxChars) : $output;
}
...
function asr_diagnostics_report(): array {
    ...
    $sections[] = ['Recent Logs', [
        'apache2 errors:',
        asr_diag_command('journalctl -u apache2 --no-pager -n 40', 5000),
        'asterisk errors:',
        asr_diag_command('journalctl -u asterisk --no-pager -n 40', 5000),
        'ASR reapply:',
        asr_diag_command('journalctl -u allscan-reimagined-reapply.service --no-pager -n 40', 5000),
    ]];
