...
rollback_on_error() {
  ...
  if [ "$CHANGES_STARTED" -eq 1 ]; then
    ...
    for helper_spec in \
      ...
      "scripts/asr-favorites-update.py:/usr/local/sbin/allscan-reimagined-favorites-update" \
      "scripts/asr-favorites-manager.py:/usr/local/sbin/allscan-reimagined-favorites-manager"; do
...
echo "Reimagined release:        v1.0.0 Beta 8"
echo
...
validate_command "Favorites update helper self-test" \
  ...
  python3 "$RELEASE_DIR/scripts/asr-favorites-manager.py" --self-test >/dev/null
...
validate_command "side-by-side runtime source self-test" \
  php "$RELEASE_DIR/scripts/asr-runtime-source-self-test.php" >/dev/null
...
echo "AllScan Reimagined:    v1.0.0 Beta 8"
