...
Gleason's AllScan. Beta 7.7 installs the current official AllScan backend at
...
This archive is **Beta 7.7** and remains a prerelease.

## What's New in Beta 7.7
...
- Beta 7.3 Bridge Cards, controls, ownership boundaries, and manual/shared
  bridge behavior remain unchanged. Beta 7.7 does not provision or adopt
...
[Read the complete Beta 7.7 release notes](release-notes/v1.0.0-beta.7.7.md).

## Install

First run `sudo -i` by itself. Confirm the new prompt begins with `root@` and
ends with `#`; stop if it does not. Then download the Beta 7.7 archive and its
...
pkg="/tmp/allscan-reimagined-1.0.0-beta.7.7.tar.gz"
checksum="${pkg}.sha256"
stage="$(mktemp -d /tmp/asr-beta-7-4-install.XXXXXX)"
base="https://github.com/ke7wil-bridge/allscan-reimagined/releases/download/v1.0.0-beta.7.7"
...
tar -xzf "$pkg" -C "$stage"
cd "$stage/allscan-reimagined-1.0.0-beta.7.7"
...
- [Beta 7.7 release notes](release-notes/v1.0.0-beta.7.7.md)
